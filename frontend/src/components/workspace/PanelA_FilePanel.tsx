  ...
  ({ width }, ref) => {
    ...
    if (!activeAgentId) {
      return (
        <div
          ref={ref}
          className="shrink-0 border-r border-(--color-border) bg-(--color-secondary) flex items-center justify-center text-[13px] text-(--color-ink-2)"
          style={{ width }}
        >
          Select an agent
        </div>
    ...
    return (
      <div
        ref={ref}
        className="shrink-0 border-r border-(--color-border) bg-(--color-secondary) flex flex-col overflow-hidden"
        style={{ width }}
      >
